...
int handler_tiempos()
{
  ...
  while(  hay_entrada(cantidad_servidores) )
  {
    ...
    for(int indice_ruido = 0; indice_ruido < cantidad_iteraciones_para_eliminar_ruido; indice_ruido++)
    {
      ...
      double tiempo_iteracion = ((double)cantidad_de_ticks_de_clock_cpu)/CLOCKS_PER_SEC;
    ...
    archivo. */
    promedio = 0;
    for(int indice_promedios = 0; indice_promedios<cantidad_iteraciones_para_eliminar_ruido; indice_promedios++)
    {
